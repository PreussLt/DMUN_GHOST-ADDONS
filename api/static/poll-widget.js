...
(function () {
  ...
  function initPoll(container) {
    ...
    let sessionId = sessionStorage.getItem(SID_KEY) || generateSid();
    ...
    async function load() {
      ...
      try {
        const res = await fetch(`${apiUrl}/api/poll/${pollId}?session_id=${sessionId}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        poll = await res.json();
        render();
      } catch (e) {
        container.innerHTML = `<div class="${NS}__error">Umfrage konnte nicht geladen werden.</div>`;
      }
    }
    ...
    function render() {
      ...
      container.innerHTML = `
        <div class="${NS}__inner">
          ...
          <div class="${NS}__body">
            ${showResults ? renderResults() : renderVoteForm(closed)}
          </div>
        </div>`;
    ...
    function renderVoteForm(closed) {
      const type = poll.multiple ? "checkbox" : "radio";
      return `
        <div class="${NS}__options">
          ${poll.options.map((o, i) => `
            <label class="${NS}__option">
              <input type="${type}" name="poll_${pollId}" value="${o.id}" id="po_${o.id}_${i}" ${closed ? "disabled" : ""}>
    ...
    async function onVote() {
      ...
      const option_ids = Array.from(inputs).map(i => parseInt(i.value));
      if (!option_ids.length) {
        container.querySelector(`.${NS}__vote-btn`).classList.add(`${NS}__vote-btn--shake`);
        setTimeout(() => container.querySelector(`.${NS}__vote-btn`)?.classList.remove(`${NS}__vote-btn--shake`), 500);
        return;
      }
      try {
        const res = await fetch(`${apiUrl}/api/poll/${pollId}/vote`, {
          ...
          body: JSON.stringify({ option_ids, session_id: sessionId }),
        });
        if (res.status === 409) {
    ...
    function animateBars() {
      container.querySelectorAll(`.${NS}__bar-fill[data-pct]`).forEach(el => {
...
.dmun-poll__option input { accent-color:var(--dp-accent); }
